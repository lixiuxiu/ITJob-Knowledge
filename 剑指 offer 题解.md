...
在一个长度为 n 的数组里的所有数字都在 ` 的范围内。数组中某些数字是重复的，但不知道有几个数字是重复的，也不知道每个数字重复几次。请找出数组中任意一个重复的数字。
...
//主要是1，找到根节点-子树范围下 前序最左是根节点(前序)
//     2，找到子树的范围-根据根节点找到中序下标，可再次限定范围（中序）
// preL和preR是 前序遍历的该子树下标范围，inL是中序遍历中左边界限
private TreeNode reConstructBinaryTree(int[] pre, int preL, int preR, int inL) {
    if (preL > preR)
        return null;
    TreeNode root = new TreeNode(pre[preL]); //根据前序的范围最左边的就是根节点
  
    int inIndex = indexForInOrders.get(root.val); // 得到根节点在中序遍历中的位置
    int leftTreeSize = inIndex - inL;   //左边树的长度就是中序遍历中 根节点位置减去最左边的位置
    
   //无论在前序还是中序中，子树的长度是一定的 
...
public TreeLinkNode GetNext(TreeLinkNode pNode) {
    if (pNode.right != null) { //右子树不为空，就是右子树的最左节点
        TreeLinkNode node = pNode.right;
        while (node.left != null)
            node = node.left;
        return node;
    } else {                  //右子树为空，就是节点往上走的第一个向右上角走的父节点
...
public int minNumberInRotateArray(int[] nums) {
    ...
    while (l < h) {
       ...
       //这里判断条件要注意，表示右半边是正常的，那么不正常的就在左半边，<=和<都可以
...
private boolean backtracking(char[][] matrix, char[] str, boolean[][] marked, int pathLen, int r, int c) {
    if (pathLen == str.length)  //达到终止条件，成功返回
        return true;
    if (r < 0 || r >= rows || c < 0 || c >= cols || matrix[r][c] != str[pathLen] || marked[r][c])                   //当前子解不满足要求，返回失败
        return false;
    marked[r][c] = true;        //else 当前子解满足要求，对该子解进行处理
  
    for (int[] n : next)        //找到下一步子解的个数，挨个进行遍历
  //运行完一个backtracking才能继续运行下一个，如果下一步的其中一种子解满足要求，成功返回，结束本方法。
        if (backtracking(matrix, str, marked, pathLen + 1, r + n[0], c + n[1])) 
            return true;  //有一个子解成功就不会往其他子解走了
  
    marked[r][c] = false;//子解遍历完成都没有满足的，就回退该解。
    return false;        //返回失败
}
...
private void dfs(boolean[][] marked, int r, int c) {
    if (r < 0 || r >= rows || c < 0 || c >= cols || marked[r][c])
        return;     //不满足条件，返回
    marked[r][c] = true;   //标记为走过了
   //不满足threshold要求，返回，但是已经把这个标记为走过了，不可再走
    if (this.digitSum[r][c] > this.threshold)  
        return;
  
    cnt++;  //满足要求，就走一步
    for (int[] n : next)  //下一步可以走的步数
...
使用回溯法得到所有的数。（dfs？因为是数组，可以直接修改其中的值，也算是一种回溯吧）
...
private void print1ToMaxOfNDigits(char[] number, int digit) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        print1ToMaxOfNDigits(number, digit + 1); //number数组是全局的，里面数字可以直接覆盖掉
    }                                           //digit是传入方法是局部变量，一个方法一个digit
}
...
public ListNode deleteNode(ListNode head, ListNode tobeDelete) {
    ...
    if (tobeDelete.next != null) {
        ...
        if (head == tobeDelete)
            // 只有一个节点
            head = null;
        else {//要删除的节点在最后一个，就要遍历一遍找到它之前的那个节点
            ListNode cur = head;
...
public boolean matchCore(char[] str, int strIndex, char[] pattern, int patternIndex) {
    ...
    //当前模式后面一个是*，且字符串第1个跟模式第1个匹配,分3种匹配模式
...
public static int longestSubStringWithoutDuplication(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            if (!map.containsKey(str.charAt(i))){
                ...
                if (map.get(str.charAt(i))<pre){
                    curLen++;
                    map.put(str.charAt(i),i);
                }else{
                    maxLen=Math.max(maxLen,curLen);
                    pre=map.get(str.charAt(i))+1;
                    map.put(str.charAt(i),i);
                    curLen=i-pre+1;
                }
            }
...
public String LeftRotateString(String str, int n) {
    ...
    return new String(chars); //chars.toString()的值不对，是描述符
}
...
[Leetco de](https://leetcode.com/problems/best-time-to-buy-and-sell-stock/description/)
